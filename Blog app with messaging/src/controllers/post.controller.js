...
export const createPost = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(
      new ApiResponse(200, { posts: createdPost }, "Post created successfully.")
    );
});
...
export const searchedBlog = asyncHandler( async(req,res)=>{
  const searchTitle = String(req.query.searchTitle);

  const searchedPosts = await Post.find({title:{$regex:searchTitle , $options:'i'}});

  const count = await Post.countDocuments({title:{$regex:searchTitle , $options:'i'}});

  return res.status(201).json(
    new ApiResponse(200,{posts:searchedPosts,count:count},"Items found")
  )
